// AUTO-SYNCED from sdk/chat-kit/registry/components/chat-pane.tsx — edit there, then run `pnpm --filter @runloop/reflex-ui sync`.
/**
 * Complete chat pane for one Reflex agent: header, live transcript,
 * composer. Drop it anywhere under a `ReflexProvider`.
 *
 * ```tsx
 * <ReflexProvider baseUrl="https://reflex.runloop.ai" apiKey={key} organizationId="my-org">
...
import { useInterruptAgent } from '../hooks/use-interrupt-agent';
import { useSendMessage } from '../hooks/use-send-message';
import { latestAgentStatus } from '../lib/event-utils';
import type { AgentTimelineDisplayItem } from '../lib/event-utils';
...
export function ChatPane({
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden rounded-xl border border-[var(--reflex-chat-border,#e5e7eb)] bg-[var(--reflex-chat-bg,#ffffff)]">
      {header !== undefined ? (
        header
      ) : (
      ...
      {footer !== undefined ? (
        ...
        <div className="border-t border-[var(--reflex-chat-border,#e5e7eb)] px-4 py-2 text-center text-xs text-[var(--reflex-chat-muted-fg,#6b7280)]">
