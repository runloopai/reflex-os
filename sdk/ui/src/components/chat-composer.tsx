// AUTO-SYNCED from sdk/chat-kit/registry/components/chat-composer.tsx — edit there, then run `pnpm --filter @runloop/reflex-ui sync`.
/**
...
export function ChatComposer({
  ...
  return (
    <form
      className="border-t border-[var(--reflex-chat-border,#e5e7eb)] p-3"
      onSubmit={(event) => {
        event.preventDefault();
        submit();
      }}
    ...
    >
      <div
        className={`rounded-2xl border bg-[var(--reflex-chat-input-bg,#ffffff)] px-3 py-2 focus-within:border-[var(--reflex-chat-accent,#4f46e5)] ${
          dragging
            ? 'border-dashed border-[var(--reflex-chat-accent,#4f46e5)]'
            : 'border-[var(--reflex-chat-border,#e5e7eb)]'
        }`}
      >
